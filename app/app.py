...
transforms = A.Compose([
    #ShiftScaleRotate(always_apply=False, p=0.3, shift_limit=(-0.05, 0.05), scale_limit=(-0.1, 0.2), rotate_limit=(-5, 5), interpolation=1, border_mode=0, value=(0, 0, 0), mask_value=None),
...
def inference(gender, image_np ):
    ...
    #del s[argmax]
    #del s[argmin]
    output = f"Predicted Bone Age: {s.mean():.1f}} y/o\ngender:{gender}"
    df = s.to_frame()
    age_range = s.mean()
    print(output)
    #print(s)
